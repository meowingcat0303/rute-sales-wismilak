...
# OPTIMASI 1: Cache data
@st.cache_data(ttl=600)
def fetch_master_data(url):
    return pd.read_csv(url)

# OPTIMASI 2: Cache rute
...
if source == "Upload Excel":
    ...
    try:
        df = fetch_master_data(MASTER_SHEET_URL).copy()
        st.sidebar.success("Master Data Terhubung!")
    except: st.sidebar.error("Gagal terhubung.")
...
if df is not None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📂 Mode A", "🚀 Mode B", "🗺️ Mode C", "📅 Mode D"])
    ...
    with tab2:
        ...
        # TAMBAHAN 1: User bisa pilih logika
        algo_mode = st.radio("Pilih Asumsi Algoritma:", ["Greedy (Nearest Neighbor)", "Clustering (Proximity Grouping)"])
        ...
        if st.button("Jalankan Optimasi"):
            with st.spinner('Menghitung Rute...'):
                ...
                # LOGIKA ASUMSI
                if algo_mode == "Clustering (Proximity Grouping)":
    ...
    with tab4:
        ...
        # TAMBAHAN 2: User bisa pilih logika
        algo_mode_d = st.radio("Pilih Asumsi:", ["Greedy (Angle Sorting)", "Clustering (Proximity Grouping)"])
        ...
        if st.button("Generate Jadwal"):
            ...
            if algo_mode_d == "Clustering (Proximity Grouping)":
                data.sort(key=lambda x: (x[lat_col], x[lon_col]))
            else:
            ...
            for i, hari in enumerate(["Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"]):
                ...
                try:
                    ...
                    while unvisited:
                        ...
                        unvisited.remove(best)
                    route_indices.append(0)
                    table_data = [{"Hari": hari, "Urutan": k+1, "Kode Customer": codes[route_indices[k+1]], "Toko": names[route_indices[k+1]], "Waktu": round(matrix[route_indices[k]][route_indices[k+1]] / 60, 2), "Navigasi": get_single_leg_link(locations[route_indices[k]][0], locations[route_indices[k]][1], locations[route_indices[k+1]][0], locations[route_indices[k+1]][1])} for k in range(len(route_indices)-1)]
                    jadwal_final[hari] = pd.DataFrame(table_data)
                except: st.error(f"Gagal rute {hari}")
